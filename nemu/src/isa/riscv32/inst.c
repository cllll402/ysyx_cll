...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
	...
	switch (type) {
		case TYPE_R: src2R(); src1R(); immR(); break;
		case TYPE_I:          src1R(); immI(); break;
		case TYPE_S: src2R(); src1R(); immS(); break;
		case TYPE_B: src2R(); src1R(); immB(); break;
...
#define INSTPAT_MATCH(s, name, type, ... ) { decode_operand(s, &rd, &src1, &src2, &imm, concat(TYPE_, type)); __VA_ARGS__ ; }
    ...
    s->dnpc = s->pc + imm;
		IFDEF(CONFIG_FTRACE, {
		    if (rd == 1) { // 根据需要修改这个条件
		        trace_func_call(s->pc, s->dnpc); // false 表示不是尾调用
		    }
		});
	);
	...
	R(rd) = s->pc + 4;
		IFDEF(CONFIG_FTRACE, {
		    // 检查 jalr 的具体值
		    if (s->isa.inst.val == 0x00008067) { // 根据需要修改这个条件
		        trace_func_ret(s->pc); // 处理返回
		    } else {
		        trace_func_call(s->pc, s->dnpc); // true 表示可能是尾调用
		    }
		});
	);
	...
	INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti     , I, R(rd) = (src1 < (unsigned)imm));
	INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu     , I, R(rd) = (src1 < (unsigned)imm));
	INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu     , R, R(rd) = (src1 < (unsigned)src2));
	...
	INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu    , B, if ((unsigned)src1 < (unsigned)src2)  s -> dnpc =  s -> pc +imm);
	...
	INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (int32_t)(((int64_t)(signed)src1 * (int64_t)(signed)src2) >> 32));
	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu   , R, R(rd) = (uint32_t)(((uint64_t)(unsigned)src1 * (uint64_t)(unsigned)src2) >> 32));
