...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { decode_operand(s, &rd, &src1, &src2, &imm, concat(TYPE_, type)); __VA_ARGS__ ; }
	...
	INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu     , B,  s -> pc += ((unsigned)src1 >= (unsigned)src2) ? imm : 4);
	
	
	//INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge     , B,  s -> pc += ((int32_t)src1 >= (int32_t)src2) ? (int32_t)imm : 4);
	//INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt     , B,  s -> pc += ((int32_t)src1 < (int32_t)src2) ? (int32_t)imm : 4);
